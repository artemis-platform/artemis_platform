defmodule AtlasWeb.Guardian.Helpers do
  import Plug.Conn

  alias AtlasWeb.ErrorView

  @doc """
  Returns the logged in user
  """
  def current_user(conn) do
    AtlasWeb.Guardian.Plug.current_resource(conn)
  end
  ...
  """
  def current_user?(conn) do
    case AtlasWeb.Guardian.Plug.current_resource(conn) do
      nil -> false
      _ -> true
    end
  end
  ...
  def redirect_to_log_in(conn) do
    path = AtlasWeb.Router.Helpers.session_path(conn, :new)
    query_params = "?redirect=#{conn.request_path}"

    conn
    |> AtlasWeb.Guardian.Plug.sign_out()
